...
#
#
# ### First attempt at the visualisation of plot4, using patchwork. 
# ### Later realised it will look better with facet (w/o repeated titles and text)
#
# ## modifying the data ##
# data2 <- data %>% mutate(lat.round=round(latitude,1)) %>% # since 0.1deg ~ 10km
#   group_by(common_name, lat.round) %>% 
#   summarise(arrival = min(week26plus), departure = max(week26plus))
# 
# data3 <- data %>% mutate(lat.round0=round(latitude)) %>% # larger scale of 1deg
#   group_by(common_name, lat.round0) %>% 
#   summarise(arrival0 = min(week26plus), departure0 = max(week26plus))
# ## ##
#  
# ## visualisation ##
# ggplot(data3, 
#          aes(y=departure0,x=lat.round0,colour=common_name)) +
#   scale_x_continuous(breaks=(seq(0,52,4))) +
#   scale_y_continuous(limits=(c(0,55)),
#                      breaks=(seq(0,55,5))) +
#   scale_colour_manual(values=c("#FFCC66","#996633","#CCCC66"),
#                       aesthetics = c("colour","fill"),
#                       labels = c("BRWa","BBFl","GrWa")) +
#   labs(title="Effect of latitude (~100km) on departure dates",
#        subtitle="Y-axis shows number of weeks after the 26th week of a year") +
#   geom_point() + geom_smooth(orientation="x", span=5, aes(fill=common_name)) +
#   geom_hline(yintercept=26) +
#   annotate("text", y=25, x=10, label = "Dec") +
#   annotate("text", y=28, x=10, label = "Jan") +
#   theme_bw() +
# ggplot(data2, aes(y=departure, x=lat.round, colour=common_name)) +
#   scale_x_continuous(breaks=(seq(0,52,4))) +
#   scale_y_continuous(limits=(c(0,55)),
#                      breaks=(seq(0,55,5))) +
#   scale_colour_manual(values=c("#FFCC66","#996633","#CCCC66"),
#                       aesthetics = c("colour","fill"),
#                       labels = c("BRWa","BBFl","GrWa")) +
#   labs(title="Effect of latitude (~10km) on departure dates",
#        subtitle="Y-axis shows number of weeks after the 26th week of a year") +
#   geom_point() + geom_smooth(orientation="x", span=0.5, aes(fill=common_name)) +
#   geom_hline(yintercept=26) +
#   annotate("text", y=25, x=10, label = "Dec") +
#   annotate("text", y=28, x=10, label = "Jan") +
#   theme_bw() +
# ggplot(data3, aes(y=arrival0, x=lat.round0, colour=common_name)) +
#   scale_x_continuous(breaks=(seq(0,52,4))) +
#   scale_y_continuous(limits=(c(0,55)),
#                      breaks=(seq(0,55,5))) +
#   scale_colour_manual(values=c("#FFCC66","#996633","#CCCC66"),
#                       aesthetics = c("colour","fill"),
#                       labels = c("BRWa","BBFl","GrWa")) +
#   labs(title="Effect of latitude (~100km) on arrival dates",
#        subtitle="Y-axis shows number of weeks after the 26th week of a year") +
#   geom_point() + geom_smooth(orientation="x", span=5, aes(fill=common_name)) +
#   geom_hline(yintercept=26) +
#   annotate("text", y=25, x=10, label = "Dec") +
#   annotate("text", y=28, x=10, label = "Jan") +
#   theme_bw() +
# ggplot(data2, aes(y=arrival, x=lat.round, colour=common_name)) +
#   scale_x_continuous(breaks=(seq(0,52,4))) +
#   scale_y_continuous(limits=(c(0,55)),
#                      breaks=(seq(0,55,5))) +
#   scale_colour_manual(values=c("#FFCC66","#996633","#CCCC66"),
#                       aesthetics = c("colour","fill"),
#                       labels = c("BRWa","BBFl","GrWa")) +
#   labs(title="Effect of latitude (~10km) on arrival dates",
#        subtitle="Y-axis shows number of weeks after the 26th week of a year") +
#   geom_point() + geom_smooth(orientation="x", span=0.5, aes(fill=common_name)) +
#   geom_hline(yintercept=26) +
#   annotate("text", y=25, x=10, label = "Dec") +
...
# 
# # was actually happy with this visualisation, but the volume of redundant text in 
# # both the code and the graph made me think of alternatives
# 
# ## ##
#
# 
...
# 
# 
# ### Tried using facet_grid but then realised cannot use multiple geom_smooth calls
# ### (need different spans for the two different latitude scales)
# 
# ## modifying the data ##
# data4 <- data %>% mutate(latitude=round(latitude,1), # since 0.1deg ~ 10km
#                          rounding = "0.1deg (~10km)") %>% 
#   group_by(common_name, rounding, latitude) %>% 
#   summarise(arrival = min(week26plus), departure = max(week26plus)) %>% 
#   pivot_longer(cols = arrival:departure,
#                names_to = "date_type", values_to = "weeks26plus")
# 
# data5 <- data %>% mutate(latitude=round(latitude), # since 1deg ~ 100km
#                          rounding = "1deg (~100km)") %>% 
#   group_by(common_name, rounding, latitude) %>% 
#   summarise(arrival = min(week26plus), departure = max(week26plus)) %>% 
#   pivot_longer(cols = arrival:departure,
#                names_to = "date_type", values_to = "weeks26plus")
# 
# data6 <- bind_rows(data4, data5) 
# ## ##
# 
# ## visualisation ##
# ggplot(data6, aes(x=latitude, y=weeks26plus, colour=common_name)) +
#   theme_bw() + theme(strip.placement = "outside") +
#   scale_x_continuous(breaks=(seq(0,52,4))) +
#   scale_y_continuous(limits=(c(0,55)),
#                      breaks=(seq(0,55,5))) +
...
#   geom_point() + 
#   geom_smooth(orientation="x", span=0.5, aes(fill=common_name)) +
#   facet_grid(rows = vars(data6$date_type), 
#              cols = vars(data6$rounding),
#              switch = "y") +
#   ggtitle("Variation of first arrival and last departure dates with latitude") 
# 
# # abandoned the attempt because facet_grid doesn't allow partial plotting
# # of geom objects 
#
# ## ##
...
### 1: more specific definition of "arrival" and "departure"


## modifying data ##

# using quantiles.
# type 7 (default) is for continuous vectors. type 3 doesn't round off. type 1 best.

# # cutting to 25-75% is too strict. this excludes 50% of observations. (see below)
#
# data7.b1 <- data %>% 
...
#   group_by(common_name, rounding, latitude) %>% 
#   summarise(n = n(),
#             arrival = unname(quantile(week26plus, probs = 0.25, type=1)), 
#             departure = unname(quantile(week26plus, probs = 0.75, type=1))) %>% 
#   pivot_longer(cols = arrival:departure,
#                names_to = "date_type", values_to = "weeks26plus") %>% 
#   mutate(date_type = factor(date_type, levels = c("departure","arrival")))
#
# # instead, only necessary to ignore the stragglers so we can try with 95%.
